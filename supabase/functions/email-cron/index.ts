...
Deno.serve(async (req) => {
  ...
  for (const prof of profiles ?? []) {
    ...
    const { data: pays } = await sb
      .from("payments")
      .select(
        "id, amount, due_date, asaas_invoice_url, contract:contracts(tenant:tenants(full_name, email))",
      )
      .eq("user_id", prof.id)
      .eq("status", "pendente")
      .eq("due_date", due)
    ...
    for (const p of pays ?? []) {
      ...
      const tpl = paymentReminderEmail({
        ...
        dueDate: p.due_date,
        invoiceUrl: p.asaas_invoice_url || undefined,
      });
      const r = await sendEmail({ to: tenant.email, ...tpl, attachments: [LOGO_ATTACHMENT] });
      if (r.error) {
        errors.push(`reminder ${p.id}: ${r.error}`);
        continue;
      }
      await sb
        .from("payments")
  ...
  for (const p of late ?? []) {
    ...
    const r = await sendEmail({ to: tenant.email, ...tpl, attachments: [LOGO_ATTACHMENT] });
    if (r.error) {
      errors.push(`overdue ${p.id}: ${r.error}`);
      continue;
    }
    await sb
      .from("payments")
