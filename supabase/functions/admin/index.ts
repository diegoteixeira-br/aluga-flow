...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "toggle_active") {
      ...
      const { error } = await sb.rpc("admin_toggle_user_active", {
        _user_id: target,
        _active: active,
      });
      if (error) throw error;
    ...
    if (action === "toggle_admin") {
      ...
      const { error } = await sb.rpc("admin_toggle_admin", {
        _user_id: target,
    ...
    if (action === "send_broadcast") {
      ...
      const html0 = broadcastEmail(subject, body);
      void html0;
      let sent = 0;
      let failed = 0;
      const errs: string[] = [];
      ...
      await admin.from("admin_email_log").insert({
        subject,
        body,
        target_plan: isUserTarget ? `user:${targetUserId}` : targetPlan,
      ...
      await admin.from("admin_logs").insert({
        ...
        details: {
          subject,
          target: isUserTarget ? `user:${targetUserId}` : targetPlan,
          sent,
          failed,
    ...
    if (action === "ai_compose") {
      ...
      const sys =
        "Você é um assistente especialista em redação de e-mails corporativos em português do Brasil para a plataforma AlugaFlow (gestão de aluguéis). Seja claro, cordial, profissional e direto. Nunca invente fatos. Não use emojis em excesso. Não inclua placeholders entre colchetes.";
      let userMsg = "";
      if (mode === "subject") {
    ...
    if (action === "legal_ai_edit") {
      ...
      if (!instruction || typeof instruction !== "string")
        return json({ error: "instruction obrigatória" }, 400);
      const LOVABLE_KEY = Deno.env.get("LOVABLE_API_KEY");
      if (!LOVABLE_KEY) return json({ error: "LOVABLE_API_KEY ausente" }, 500);
