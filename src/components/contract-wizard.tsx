...
const TEMPLATES: Array<{ id: TemplateId; label: string; desc: string }> = [
  {
    id: "editor_dinamico",
    label: "Editor com campos dinâmicos",
    desc: "Edite o texto livremente, insira variáveis [token] e pré-visualize com os dados preenchidos.",
  },
  {
    id: "padrao_11",
    label: "Padrão (11 cláusulas, Lei 8.245/91)",
    desc: "Modelo enxuto, cobre obrigações essenciais.",
  },
  {
    id: "completo_20",
    label: "Locação completo (20 cláusulas) — Residencial/Comercial",
    desc: "Modelo robusto com LGPD, sinistros, sublocação, alienação e foro.",
  },
  {
    id: "residencial_20",
    label: "Locação Residencial (20 cláusulas)",
    desc: "Modelo específico residencial com cláusulas estendidas.",
  },
];
...
export function ContractWizard({
  ...
  const [signatureLinks, setSignatureLinks] = useState<
    Array<{ role: string; name: string; email: string; url: string }>
  >([]);
  ...
  const { data: properties = [] } = useQuery({
    ...
    queryFn: async () =>
      (
        await supabase
          .from("properties")
          .select("id, nickname, address, city, state, type")
          .order("nickname")
      ).data ?? [],
  });
  const { data: tenants = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("tenants").select("id, full_name, cpf, email").order("full_name"))
        .data ?? [],
  });
  ...
  const stepValidators: (() => string | null)[] = [
    ...
    () => {
      try {
        ...
        return (e as z.ZodError).errors[0]?.message ?? "Preencha os campos";
      }
    },
    () => {
    ...
    () => {
      if (
        ...
        (state.guarantee_months < 1 || state.guarantee_months > 3)
      )
        return "Caução: 1 a 3 meses";
      if (state.guarantee_type === "fiador" && !state.add_guarantor)
        return "Volte ao passo 3 e adicione um fiador";
      return null;
    },
    () => null,
    () => null,
    ...
    () => ({
      ...
      guarantor: state.add_guarantor
        ? {
            ...
            cpf: state.guarantor_cpf,
            rg: state.guarantor_rg,
            email: state.guarantor_email,
            phone: state.guarantor_phone,
            address: state.guarantor_address,
          }
        : null,
      start_date: state.start_date,
      end_date: state.end_date,
  ...
  function previewPDF() {
    ...
    if (templateId === "completo_20") {
      gerarContratoLocacaoCompleto(contractPayload, ownerProfile).save(
        `contrato-${name.replace(/\s+/g, "-").toLowerCase()}.pdf`,
      );
    } else if (templateId === "residencial_20") {
      gerarContratoResidencial(contractPayload, ownerProfile).save(
        `contrato-${name.replace(/\s+/g, "-").toLowerCase()}.pdf`,
      );
    } else {
    ...
    mutationFn: async () => {
      ...
      if (state.payment_method === "asaas" && payments.length > 0) {
        try {
          ...
          asaasCreated = r.created;
          asaasFailed = r.failed;
          asaasErr = r.errors.join(" | ");
        } catch (e) {
          asaasErr = (e as Error).message;
          asaasFailed = payments.length;
        }
      }
  ...
  const sendInvites = useMutation({
    mutationFn: async (contractId: string) => {
      ...
      if (signers.length < 2)
        throw new Error("Cadastre email do proprietário (Configurações) e inquilino para enviar.");
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs">
            {STEPS.map((s, i) => (
              ...
              >
                <span
                  className={`flex h-5 w-5 items-center justify-center rounded-full text-[10px] ${i < step ? "bg-primary text-primary-foreground" : i === step ? "border-2 border-primary" : "border"}`}
                >
        ...
        <div className="mt-4 min-h-[300px]">
          {step === 0 && (
            <StepProperty
              ...
              onChange={(v) => patch("property_id", v)}
            />
          )}
        ...
        <div className="mt-6 flex justify-between border-t pt-4">
          <Button
            ...
            variant="outline"
            onClick={back}
            disabled={step === 0 || saveContract.isPending}
          >
  ...
  if (properties.length === 0) {
    return (
      <p className="text-center text-sm text-muted-foreground py-10">
        Cadastre um imóvel antes de criar um contrato.
      </p>
    );
  }
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Selecione o imóvel que será locado neste contrato.
      </p>
...
function StepDetails({
  ...
  function removeExtra(i: number) {
    patch(
      "extra_charges",
      state.extra_charges.filter((_, idx) => idx !== i),
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="space-y-1">
          ...
          <Select
            ...
            onValueChange={(v) => patch("contract_type", v as WizardState["contract_type"])}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            max={120}
            value={state.months}
            onChange={(e) => patch("months", Number(e.target.value) || 0)}
          />
        </div>
        <div className="space-y-1">
          <Label>Início *</Label>
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            max={28}
            value={state.due_day}
            onChange={(e) => patch("due_day", Number(e.target.value) || 1)}
          />
        </div>
        <div className="space-y-1">
          <Label>Índice de reajuste</Label>
          <Select
            value={state.adjustment_index}
            onValueChange={(v) => patch("adjustment_index", v as WizardState["adjustment_index"])}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="space-y-1">
          ...
          <Input
            ...
            min={1}
            max={60}
            value={state.adjustment_frequency_months}
            onChange={(e) => patch("adjustment_frequency_months", Number(e.target.value) || 12)}
          />
        </div>
      ...
      <div className="space-y-2 pt-2">
        ...
        <RadioGroup
          ...
          onValueChange={(v) => patch("payment_method", v as WizardState["payment_method"])}
          className="grid gap-2 sm:grid-cols-2"
        >
          <label
            className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-accent ${state.payment_method === "pix" ? "border-primary ring-2 ring-primary/30" : ""}`}
          >
            <RadioGroupItem value="pix" className="mt-1" />
            <div>
              <p className="font-medium text-sm">PIX / Transferência</p>
              <p className="text-xs text-muted-foreground">
                Cobrança manual. Você marca cada pagamento como pago.
              </p>
            </div>
          </label>
          <label
            className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-accent ${state.payment_method === "asaas" ? "border-primary ring-2 ring-primary/30" : ""}`}
          >
            <RadioGroupItem value="asaas" className="mt-1" />
            <div>
              <p className="font-medium text-sm">Boleto ASAAS (automático)</p>
              <p className="text-xs text-muted-foreground">
                Ao salvar, cria cliente e gera cobranças para todo o período. Boleto enviado ao
                e-mail do inquilino.
              </p>
            </div>
          </label>
    ...
    <div className="space-y-5">
        ...
        <Card>
          <CardContent className="p-3 text-sm space-y-1">
            <p>
              ...
              {owner?.full_name ?? "— (preencha em Configurações)"}
            </p>
            <p>
              <span className="text-muted-foreground">CPF:</span> {owner?.cpf ?? "—"}
            </p>
            <p>
              <span className="text-muted-foreground">E-mail:</span> {owner?.email ?? "—"}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        ...
        <Select value={state.tenant_id} onValueChange={(v) => patch("tenant_id", v)}>
          <SelectTrigger>
            <SelectValue placeholder="Selecione um inquilino cadastrado" />
          </SelectTrigger>
          <SelectContent>
            {tenants.map((t) => (
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <Button
            type="button"
            size="sm"
            variant={state.add_guarantor ? "secondary" : "outline"}
            onClick={() => patch("add_guarantor", !state.add_guarantor)}
          >
        ...
        {state.add_guarantor && (
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              ...
              onChange={(e) => patch("guarantor_name", e.target.value)}
            />
            <Input
              placeholder="CPF *"
              value={state.guarantor_cpf}
              onChange={(e) => patch("guarantor_cpf", e.target.value)}
            />
            <Input
              placeholder="RG"
              value={state.guarantor_rg}
              onChange={(e) => patch("guarantor_rg", e.target.value)}
            />
            <Input
              placeholder="Telefone"
              value={state.guarantor_phone}
              onChange={(e) => patch("guarantor_phone", e.target.value)}
            />
            <Input
              placeholder="E-mail"
              type="email"
              className="sm:col-span-2"
              value={state.guarantor_email}
              onChange={(e) => patch("guarantor_email", e.target.value)}
            />
            <Input
              placeholder="Endereço"
              className="sm:col-span-2"
              value={state.guarantor_address}
              onChange={(e) => patch("guarantor_address", e.target.value)}
            />
          </div>
        )}
      </div>
...
function StepGuarantee({
  ...
  return (
    <div className="space-y-4">
      ...
      <RadioGroup
        value={state.guarantee_type}
        onValueChange={(v) => patch("guarantee_type", v as WizardState["guarantee_type"])}
        className="space-y-2"
      >
        {[
          { v: "sem_garantia", l: "Sem garantia", d: "Locação sem exigência de garantia." },
          {
            v: "fiador",
            l: "Fiador",
            d: "Garantia prestada por terceiro (cadastrado no passo 3).",
          },
          {
            v: "caucao",
            l: "Caução em dinheiro",
            d: "Depósito de 1 a 3 aluguéis (art. 38 Lei 8.245/91).",
          },
          {
            v: "seguro_fianca",
            l: "Seguro fiança",
            d: "Apólice contratada pelo locatário em seguradora.",
          },
        ].map((o) => (
          <label
            key={o.v}
            className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-accent ${state.guarantee_type === o.v ? "border-primary ring-2 ring-primary/30" : ""}`}
          >
      ...
      {state.guarantee_type === "caucao" && (
        <div className="space-y-1 max-w-xs">
          ...
          <Input
            ...
            min={1}
            max={3}
            value={state.guarantee_months}
            onChange={(e) =>
              patch("guarantee_months", Math.min(3, Math.max(1, Number(e.target.value) || 1)))
            }
          />
          <p className="text-xs text-muted-foreground">
            Total: {formatBRL((state.guarantee_months || 0) * state.rent_amount)}
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Escolha o modelo de contrato. As variáveis serão preenchidas automaticamente com os dados
        ...
        <>
          <Card>
            <CardContent className="p-4 text-sm space-y-1">
              <p>
                <b>Imóvel:</b> {payload.property?.nickname} — {payload.property?.address}
              </p>
              <p>
                <b>Inquilino:</b> {payload.tenant?.full_name}
              </p>
              {payload.guarantor?.name && (
                <p>
                  <b>Fiador:</b> {payload.guarantor.name}
                </p>
              )}
              <p>
                <b>Vigência:</b> {formatDate(payload.start_date)} a {formatDate(payload.end_date)}
              </p>
              <p>
                <b>Aluguel:</b> {formatBRL(payload.rent_amount)} — venc. dia {payload.due_day}
              </p>
              {payload.extra_charges && payload.extra_charges.length > 0 && (
                <p>
                  <b>Cobranças extras:</b>{" "}
                  {payload.extra_charges
                    .map((e) => `${e.label} (${formatBRL(e.amount)})`)
                    .join(", ")}
                </p>
              )}
              <p>
                <b>Garantia:</b> {payload.guarantee_type}
              </p>
            </CardContent>
          </Card>
...
function StepSignature({
    ...
    return (
      <div className="space-y-4">
        <h3 className="font-semibold text-green-700">
          Contrato criado — convites de assinatura prontos
        </h3>
        <p className="text-sm text-muted-foreground">
          Envie os links abaixo para cada signatário. (Quando o domínio de email do projeto for
          configurado, esses links serão enviados automaticamente por email.)
        </p>
        <div className="space-y-2">
          {links.map((l) => (
            <Card key={l.url}>
  ...
  return (
    <div className="space-y-4">
      ...
      <RadioGroup
        value={state.signature_mode}
        onValueChange={(v) => patch("signature_mode", v as WizardState["signature_mode"])}
        className="space-y-2"
      >
        <label
          className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-accent ${state.signature_mode === "manual" ? "border-primary ring-2 ring-primary/30" : ""}`}
        >
          <RadioGroupItem value="manual" className="mt-1" />
          <div>
            <p className="font-medium">Manual</p>
            <p className="text-xs text-muted-foreground">
              Baixar o PDF e coletar as assinaturas presencialmente.
            </p>
          </div>
        </label>
        <label
          className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer hover:bg-accent ${state.signature_mode === "eletronica" ? "border-primary ring-2 ring-primary/30" : ""}`}
        >
          <RadioGroupItem value="eletronica" className="mt-1" />
          <div>
            <p className="font-medium">Eletrônica</p>
            <p className="text-xs text-muted-foreground">
              Gera um link único por signatário. Cada um confirma identidade (nome completo + CPF) e
              assina digitalmente, com registro de IP e timestamp (MP 2.200-2/2001).
            </p>
          </div>
        </label>
      </RadioGroup>
      ...
      {state.signature_mode === "manual" ? (
        <Button type="button" onClick={onManual} disabled={saving} className="w-full">
          {saving ? (
            ...
            <>
              <FileDown className="h-4 w-4" /> Criar contrato e baixar PDF
            </>
          )}
        </Button>
      ) : (
        ...
        >
          {saving || sending ? (
            <>
              <Loader2 className="h-4 w-4 animate-spin" /> Gerando convites…
            </>
          ) : (
            <>
              <Send className="h-4 w-4" /> Criar contrato e gerar links de assinatura
            </>
          )}
        </Button>
      )}
