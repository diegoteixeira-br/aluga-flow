...
export function ContractEditor({
  ...
  const values = useMemo(() => (owner ? buildTokenValues(payload, owner) : {}), [payload, owner]);
  ...
  function downloadPDF() {
    ...
    doc.save(
      `contrato-${(payload.property?.nickname ?? "documento").replace(/\s+/g, "-").toLowerCase()}.pdf`,
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
        <strong>Atenção:</strong> os modelos são apenas um ponto de partida. Leia, edite e adapte o
        texto à realidade do seu negócio antes de enviar para assinatura. Recomendamos a revisão por
        ...
        <Card className="h-fit">
          <CardContent className="p-2">
            <p className="px-2 py-1 text-xs text-muted-foreground">
              Arraste os campos para a área de texto ao lado.
            </p>
            ...
            <div className="mt-2 px-1 space-y-2">
              ...
              >
                {preview ? (
                  <>
                    <EyeOff className="h-4 w-4" /> Editar template
                  </>
                ) : (
                  <>
              ...
              <Button
                ...
                variant="ghost"
                className="w-full"
                onClick={() => onChange(TEMPLATE_LOCACAO_DINAMICO)}
              >
