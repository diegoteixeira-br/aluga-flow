...
function SignPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!agreed) {
      toast.error("Confirme a leitura do contrato");
      return;
    }
    setSubmitting(true);
    try {
  ...
  if (done || view.signature.signed_at)
    return (
        ...
        <Card className="max-w-md">
          <CardContent className="p-8 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Obrigado, {view.signature.name}. Sua assinatura foi registrada com sucesso. Você pode
              fechar esta página.
            </p>
          </CardContent>
        </Card>
    ...
    <div className="min-h-screen bg-muted/30 py-8 px-4">
      <div className="mx-auto max-w-3xl space-y-4">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            AlugaFlow — Lei nº 8.245/91 · MP 2.200-2/2001
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-5 space-y-2 text-sm">
            <div className="flex items-center gap-2 font-semibold">
              <FileText className="h-4 w-4" /> Contrato de locação {c.contract_type}
            </div>
            <p>
              <b>Imóvel:</b> {c.property?.nickname} — {c.property?.address}
              {c.property?.city ? `, ${c.property.city}/${c.property.state ?? ""}` : ""}
            </p>
            <p>
              <b>Locador:</b> {view.owner?.full_name ?? "—"}
            </p>
            <p>
              <b>Locatário:</b> {c.tenant?.full_name ?? "—"}
            </p>
            <p>
              <b>Vigência:</b> {formatDate(c.start_date)} a {formatDate(c.end_date)}
            </p>
            <p>
              <b>Aluguel:</b> {formatBRL(c.rent_amount)} — vencimento todo dia {c.due_day}
            </p>
            <p className="text-xs text-muted-foreground pt-2">
              O contrato completo, com todas as cláusulas, foi previamente enviado/disponibilizado
              pelo locador. Ao assinar você confirma ter lido e concordado com seus termos
              integrais.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5 space-y-3">
            <p className="text-sm">
              <b>Você está assinando como:</b>{" "}
              <span className="capitalize">{view.signature.role}</span> — {view.signature.name}
            </p>
            <form onSubmit={submit} className="space-y-3">
              ...
              <label className="flex items-start gap-2 text-sm">
                ...
                />
                <span>
                  Declaro que li e concordo integralmente com os termos do contrato apresentado, e
                  que as informações acima são verdadeiras. Reconheço a validade jurídica desta
