  ...
  head: () => ({
    meta: [
      { title: "Para proprietários — AlugaFlow" },
      {
        name: "description",
        content:
          "Gestão completa de aluguéis: imóveis, inquilinos, contratos e cobrança automática. Conheça a ferramenta e os planos.",
      },
...
const FEATURES = [
  {
    icon: Building2,
    title: "Imóveis",
    desc: "Cadastre e organize sua carteira com fotos e detalhes.",
  },
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-72">
                ...
                <nav className="mt-4 flex flex-col gap-1">
                  <SheetClose asChild>
                    <Link to="/" className="rounded-md px-3 py-2 text-sm hover:bg-muted">
                      Imóveis
                    </Link>
                  </SheetClose>
                  <SheetClose asChild>
      ...
      <section className="bg-gradient-to-br from-primary/10 via-background to-background py-16 md:py-24">
        <div className="mx-auto max-w-5xl px-4 text-center">
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight md:text-5xl">
            Gestão completa dos seus aluguéis
          </h1>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-muted-foreground">
            Da captação do inquilino à cobrança do aluguel.
          </p>
        </div>
        ...
        <div className="mx-auto max-w-6xl px-4">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {plans.map((p, idx) => {
              ...
              const baseBenefits = (
                Array.isArray(p.benefits) ? (p.benefits as string[]) : []
              ...
              const benefits =
                extraUsers > 0
                  ? [
                      ...baseBenefits,
                      `Múltiplos acessos (+${extraUsers} usuário${extraUsers > 1 ? "s" : ""})`,
                    ]
                  : baseBenefits;
              ...
              return (
                <div
                  key={p.id}
                  className={`rounded-lg border bg-card p-6 ${highlight ? "border-primary shadow-lg ring-1 ring-primary" : ""}`}
                >
                  {highlight && (
                    <div className="mb-3 inline-block rounded-full bg-primary px-3 py-1 text-xs font-semibold text-primary-foreground">
                      Mais popular
                    </div>
                  )}
                  ...
                  <div className="mt-3 flex items-baseline gap-2">
                    <span className="text-3xl font-bold">
                      {isFree ? "R$ 0" : formatBRL(effective)}
                    </span>
                    <span className="text-sm text-muted-foreground">
                      {isFree ? "para sempre" : "/mês"}
                    </span>
                    {promo && (
      ...
      <footer className="border-t bg-muted/30">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 px-4 py-6 text-xs text-muted-foreground sm:flex-row">
          <div className="flex items-center gap-2">
            <BrandLogo size={20} /> © 2025 AlugaFlow
          </div>
