...
function AdminFinance() {
  ...
  const save = async () => {
    ...
    setOpen(false);
    setForm({ kind: "receita", entry_date: todayISO() });
    load();
  };
  ...
  const exportCSV = () => {
    const csv = [
      ...
      ...rows.map(
        (r) => `${r.entry_date};${r.kind};"${r.description}";${r.category ?? ""};${r.amount}`,
      ),
    ].join("\n");
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-emerald-600">{formatBRL(monthRevenue)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{formatBRL(monthCost)}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatBRL(monthRevenue - monthCost)}</div>
          </CardContent>
        </Card>
      </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <Badge variant={r.kind === "receita" ? "default" : "destructive"}>
                        {r.kind}
                      </Badge>
                    </TableCell>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  value={form.kind}
                  onValueChange={(v) => setForm({ ...form, kind: v as "receita" })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
