  ...
  return (
    <div className="space-y-4">
      ...
      <Alert>
        ...
        <AlertDescription>
          Os e-mails são enviados imediatamente pelo Resend usando o remetente configurado em{" "}
          <code>RESEND_FROM_EMAIL</code>. Você pode disparar para todos, para um plano específico ou
          para um único usuário pesquisando pelo nome/e-mail.
        </AlertDescription>
      </Alert>
        ...
        <CardContent className="space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Personalize usando variáveis: <code>{"{{nome}}"}</code>,{" "}
              <code>{"{{primeiro_nome}}"}</code>, <code>{"{{email}}"}</code>,{" "}
              <code>{"{{plano}}"}</code>. O logo da AlugaFlow é incluído automaticamente no topo.
            </p>
          </div>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => {
                  ...
                  if (v !== "user") setSelectedUser(null);
                }}
              >
                <SelectTrigger>
                  <SelectValue />
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {filteredUsers.map((u) => (
                            ...
                            >
                              <Check
                                className={`mr-2 h-4 w-4 ${selectedUser?.id === u.id ? "opacity-100" : "opacity-0"}`}
                              />
                ...
                history.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="capitalize">
                      {r.target_plan.startsWith("user:") ? "Usuário específico" : r.target_plan}
                    </TableCell>
