...
function AdminUsers() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{filtered.length} usuários</CardTitle>
        </CardHeader>
        <CardContent>
          <Input
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      <Select
                        ...
                        onValueChange={(v) =>
                          m(
                            () => setPlan({ data: { userId: r.id, plan: v as "free" } }),
                            "Plano atualizado",
                          )
                        }
                      >
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <Button
                          size="sm"
                          variant="ghost"
                          title={r.is_admin ? "Revogar admin" : "Tornar admin"}
                          onClick={() =>
                            m(
                              () => toggleAdmin({ data: { userId: r.id, makeAdmin: !r.is_admin } }),
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          title={r.active ? "Bloquear" : "Reativar"}
                          onClick={() =>
                            m(
                              () => toggleActive({ data: { userId: r.id, active: !r.active } }),
                              "Status atualizado",
                            )
                          }
                        >
